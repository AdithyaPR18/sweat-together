...
export default function Track({ toast, guest }: { toast: (m: string, t?: "ok" | "error") => void; guest: boolean }) {
  ...
  const thumbsTouch = 0.1; // normalized wrist distance for "touch"
  // -------------------------------------------------------------
  ...
  const onResults = (res: Results) => {
    ...
    const R_HIP = lm[24]; // use hip rather than "thigh" index for torso check
    ...
    ctx.lineWidth = 1;
    // roundRect isn't in all canvases; fallback if missing
    // @ts-ignore
    if (ctx.roundRect) {
      // @ts-ignore
      ctx.beginPath(); ctx.roundRect(12, canvas.height - 44, 320, 28, 10); ctx.fill(); ctx.stroke();
    } else {
      ctx.fillRect(12, canvas.height - 44, 320, 28);
      ctx.strokeRect(12, canvas.height - 44, 320, 28);
    }
  ...
  const save = async () => {
    ...
    // Hook up to your backend save endpoint if desired
    toast("Session saved");
  };
